...
void MapROS::init() {
  ...
  node_.param("map_ros/depth_filter_maxdist", depth_filter_maxdist_, -1.0);//5.0
  node_.param("map_ros/depth_filter_mindist", depth_filter_mindist_, -1.0);//0.2
  node_.param("map_ros/depth_filter_margin", depth_filter_margin_, -1);//2
  node_.param("map_ros/k_depth_scaling_factor", k_depth_scaling_factor_, -1.0);//1000.0
  node_.param("map_ros/skip_pixel", skip_pixel_, -1);//2

  node_.param("map_ros/esdf_slice_height", esdf_slice_height_, -0.1);//0.3
  node_.param("map_ros/visualization_truncate_height", visualization_truncate_height_, -0.1);//10.09
  node_.param("map_ros/visualization_truncate_low", visualization_truncate_low_, -0.1);//-2
  node_.param("map_ros/show_occ_time", show_occ_time_, false);//false
  node_.param("map_ros/show_esdf_time", show_esdf_time_, false);//fasle
  node_.param("map_ros/show_all_map", show_all_map_, false);//true
...
void MapROS::visCallback(const ros::TimerEvent& e) {
  publishMapLocal();//发布局部地图
  if (show_all_map_) {
    // Limit the frequency of all map
    static double tpass = 0.0;
    tpass += (e.current_real - e.last_real).toSec();//实际的时间间隔
    if (tpass > 0.1) {//以10hz的频率发送全局地图
      publishMapAll();//发布全局地图
      tpass = 0.0;
    }
  }
  // publishUnknown();
...
void MapROS::updateESDFCallback(const ros::TimerEvent& /*event*/) {
  if (!esdf_need_update_) return;//每传来一帧点云都会为true
  auto t1 = ros::Time::now();

  map_->updateESDF3d();//具体如何更新看不懂，功能是更新局部的ESDF地图
  esdf_need_update_ = false;
...
void MapROS::depthPoseCallback(const sensor_msgs::ImageConstPtr& img,
  ...
  camera_pos_(2) = pose->pose.position.z;//定义相机位置
  if (!map_->isInMap(camera_pos_))  // exceed mapped region
    return;//如果相机位置超出地图范围，则返回不进行处理
  ...
  proessDepthImage();//处理每一帧深度图像，得到深度点云，并将其放置在point_cloud_.points中
  map_->inputPointCloud(point_cloud_, proj_points_cnt, camera_pos_);//proj_points_cnt表示深度点云中点的数量
//以上语句将每帧深度点云输入到SDF地图中，并根据点云点更新局部地图更新范围，且设置每个点的占据概率，并令local_updated_为true
  if (local_updated_) {//输入点云之后，local_updated_为true
    map_->clearAndInflateLocalMap();//膨胀传来的点云图
...
void MapROS::proessDepthImage() {
  proj_points_cnt = 0;//每一帧图像来时就将其清零

  uint16_t* row_ptr;
  int cols = depth_image_->cols;
  int rows = depth_image_->rows;
  double depth;
  Eigen::Matrix3d camera_r = camera_q_.toRotationMatrix();//旋转矩阵
  ...
  for (int v = depth_filter_margin_; v < rows - depth_filter_margin_; v += skip_pixel_) {
    ...
    for (int u = depth_filter_margin_; u < cols - depth_filter_margin_; u += skip_pixel_) {
      depth = (*row_ptr) * inv_factor;//具体如何计算深度值不太懂
      ...
      if (*row_ptr == 0 || depth > depth_filter_maxdist_)
        depth = depth_filter_maxdist_;//当深度值超过5m时，将深度值置为5m
      else if (depth < depth_filter_mindist_)//0.2,当深度值小于0.2m时，将其舍弃
        continue;
      ...
      auto& pt = point_cloud_.points[proj_points_cnt++];//point_cloud_.points里面装的是每一帧的世界坐标系下的深度点
      pt.x = pt_world[0];
      pt.y = pt_world[1];
      pt.z = pt_world[2];
    }
...
void MapROS::publishMapAll() {
  int all_num,count_=0;
  Eigen::Vector3d min_(1000,1000,1000),max_(-1000,-1000,-1000);
  pcl::PointXYZ pt;
    ...
    for (int y = map_->mp_->box_min_(1) /* + 1 */; y < map_->mp_->box_max_(1); ++y)
      for (int z = map_->mp_->box_min_(2) /* + 1 */; z < map_->mp_->box_max_(2); ++z) {
        if (map_->md_->occupancy_buffer_[map_->toAddress(x, y, z)] > map_->mp_->min_occupancy_log_) {
          ...
          for (int k = 0; k < 3; ++k) {
          min_[k] = min(pos[k], min_[k]);
          max_[k] = max(pos[k], max_[k]);
          }
          cloud1.push_back(pt);
        }
      }
  cloud1.width = cloud1.points.size();
  all_num=(map_->mp_->box_maxd_(0)-map_->mp_->box_mind_(0))*(map_->mp_->box_maxd_(1)-map_->mp_->box_mind_(1))*100;
  cloud1.height = 1;
  ...
  local_map1=(max_[0]-min_[0])*(max_[1]-min_[1])*(max_[2]-min_[2]);
  double cover;
  cover=count_*1.0/all_num;
  //std::cout<<"地图覆盖率： "<<cover<<std::endl;
  ...
  ofstream file("/home/dtpmh/exploration_ws/test/"
                "curve1.txt",
                ios::app);
  file << "time: " << time_now << ",vol: " << known_volumn << std::endl;
    //std::cout<<"体素占据： "<<known_volumn<<std::endl;
...
void MapROS::publishMapLocal() {
  ...
  Eigen::Vector3i min_cut = map_->md_->local_bound_min_;//每帧点云传来时，都会更新
