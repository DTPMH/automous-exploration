...
void BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
  ...
  dim_ = control_points_.cols();//维数，即三维坐标
  if (dim_ == 1)
    order_ = 3;
  else
    order_ = bspline_degree_;
  point_num_ = control_points_.rows();//点的数量
  optimize_time_ = cost_function_ & MINTIME;
  variable_num_ = optimize_time_ ? dim_ * point_num_ + 1 : dim_ * point_num_;//variable_num_表示变量的数量
  if (variable_num_ <= 0) {
