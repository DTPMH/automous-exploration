...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  planner_manager_->initPlanModules(nh);//初始化地图，路径规划，优化，以及前沿搜索的参数
  edt_environment_ = planner_manager_->edt_environment_;//赋值环境
  sdf_map_ = edt_environment_->sdf_map_;//赋值地图
  frontier_finder_.reset(new FrontierFinder(edt_environment_, nh));//reset智能指针，
  ...
  nh.param("exploration/refine_local", ep_->refine_local_, true);//true
  nh.param("exploration/refined_num", ep_->refined_num_, -1);//7
  nh.param("exploration/refined_radius", ep_->refined_radius_, -1.0);//5.0
  nh.param("exploration/top_view_num", ep_->top_view_num_, -1);//15
  nh.param("exploration/max_decay", ep_->max_decay_, -1.0);//0.8
  nh.param("exploration/tsp_dir", ep_->tsp_dir_, string("null"));//(find lkh_tsp_solver)/resource
  nh.param("exploration/relax_time", ep_->relax_time_, 1.0);//1.0

  nh.param("exploration/vm", ViewNode::vm_, -1.0);//最大速度
  nh.param("exploration/am", ViewNode::am_, -1.0);//最大加速度
  nh.param("exploration/yd", ViewNode::yd_, -1.0);//80°，真值为弧度
  nh.param("exploration/ydd", ViewNode::ydd_, -1.0);//90°，真值为弧度
  nh.param("exploration/w_dir", ViewNode::w_dir_, -1.0);//1.5



  ViewNode::astar_.reset(new Astar);//astar_是子类ViewNode中指向Astar类的一个类指针
  ViewNode::astar_->init(nh, edt_environment_);//对应astar2.cpp
  ViewNode::map_ = sdf_map_;//与上同理

  double resolution_ = sdf_map_->getResolution();//0.1
  Eigen::Vector3d origin, size;
  sdf_map_->getRegion(origin, size);//设置地图的大小
...
int FastExplorationManager::planExploreMotion(
  ...
  // 开始搜索前沿
  frontier_finder_->searchFrontiers();//搜搜前沿，并计算每个前沿对应的中心点以及约束框，并将其放入tmp_frontiers_中

  double frontier_time = (ros::Time::now() - t1).toSec();//frontier_time表示前沿搜索所用的时间
  t1 = ros::Time::now();

// Find viewpoints (x,y,z,yaw) for all frontier clusters and get visible ones' info
  frontier_finder_->computeFrontiersToVisit();//计算每个前沿的观测点对应的坐标以及对应的偏航角，并对观测点进行排序（以观测到前沿点的数量为基准）
  frontier_finder_->getFrontiers(ed_->frontiers_);//将每个前沿的前沿坐标点放置在ed_->frontiers_（三维变量：前沿索引，前沿点索引，前沿点坐标）中
  frontier_finder_->getFrontierBoxes(ed_->frontier_boxes_);//将每个前沿的约束框对应的中心点以及大小放入ed_->frontier_boxes_（二维：前沿索引，约束框的中心以及大小）中
  frontier_finder_->getDormantFrontiers(ed_->dead_frontiers_);//将没有观测点的前沿放入ed_->dead_frontiers_中

  if (ed_->frontiers_.empty()) {//如果没有前沿点，则说明没找到前沿
  ...
  //ed_->points_存储的是所有前沿的对应观测点的坐标
  //ed_->yaws_存储的是所有前沿的对应观测点的偏航角
  //ed_->averages_存储的是所有前沿的中心点坐标
  for (int i = 0; i < ed_->points_.size(); ++i)
    ed_->views_.push_back(
        ed_->points_[i] + 2.0 * Vector3d(cos(ed_->yaws_[i]), sin(ed_->yaws_[i]), 0));

  double view_time = (ros::Time::now() - t1).toSec();//view_time表示前沿搜索用到的时间
  ROS_WARN(
...
/******************************************************原本的***********************************************/
/*
  if (ed_->points_.size() > 1) {//如果观测点的数量大于1个
    ...
    vector<int> indices;
    findGlobalTour(pos, vel, yaw, indices);//indices表示全局前沿的顺序
...
//
  ...
  } else if (ed_->points_.size() == 1) {//如果全局只有一个观测点
...
/***************************************************************************************************************************/
  ...
  double time_lb = min(diff, 2 * M_PI - diff) / ViewNode::yd_;//80°对应的弧度
  ...
  ed_->path_next_goal_ = planner_manager_->path_finder_->getPath();//得到搜索的简单轨迹，里面包含的只有路径点坐标
  shortenPath(ed_->path_next_goal_);//只留下重要的轨迹点

  const double radius = 5.0;//5.0
  const double len = Astar::pathLength(ed_->path_next_goal_);//计算路径得到长度
  if (len < 1.5) {//如果路径长度小于1.5，则说明下一个观测点十分近，因此只用到基于路径点的优化即可

    planner_manager_->planExploreTraj(ed_->path_next_goal_, vel, acc, time_lb);//利用多项式轨迹进行优化
    std::cout << "Close goal." << std::endl;
  } else {
    if (len > radius) {//如果轨迹长度大于5.0，说明下一个观测点很远，因此需要从路径中选择一个中间点作为下一个目标，也是为了防止出现无法到达的目标点
//原理是从A*搜索的路径中找到一个距离当前点大约在5m的中间点作为下一个目标点
      double len = 0.0;
      ...
      ed_->next_goal_ = next_pos;//如果距离在1.5-5.0之间，直接使用观测点作为目标点

    if (!planner_manager_->kinodynamicReplan(//进行动力学约束的A*算法找轨迹
            pos, vel, acc, ed_->next_goal_, Vector3d(0, 0, 0), time_lb))
      return FAIL;
  }

  if (planner_manager_->local_data_.position_traj_.getTimeSum() < time_lb - 0.1)//如果搜索出的轨迹的总时间间隔小于了time_lb
    ROS_ERROR("Lower bound not satified!");//则不满足最小约束

  planner_manager_->planYawExplore(yaw, next_yaw, true, ep_->relax_time_);//利用每段轨迹的连接处的偏航角大小，进行偏航角轨迹优化
  //planner_manager_->planYaw(yaw);//, next_yaw, true, ep_->relax_time_);//利用每段轨迹的连接处的偏航角大小，进行偏航角轨迹优化

  double traj_plan_time = (ros::Time::now() - t1).toSec();//traj_plan_time表示规划轨迹的时间
  t1 = ros::Time::now();
