...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  nh.param("manager/max_vel", pp_.max_vel_, -1.0);//自己设置的最大速度
  nh.param("manager/max_acc", pp_.max_acc_, -1.0);//自己设置的最大加速度
  nh.param("manager/max_jerk", pp_.max_jerk_, -1.0);//最大jerk,(jerk是加速度的导数)
  nh.param("manager/accept_vel", pp_.accept_vel_, pp_.max_vel_ + 0.5);//可接受的速度为最大速度+0.5
  nh.param("manager/accept_acc", pp_.accept_acc_, pp_.max_acc_ + 0.5);//可接受的加速度为最大加速度+0.5
  nh.param("manager/max_yawdot", pp_.max_yawdot_, -1.0);//最大的偏航角速率，没有设置默认为-1.0
  nh.param("manager/dynamic_environment", pp_.dynamic_, -1);//0
  nh.param("manager/clearance_threshold", pp_.clearance_, -1.0);//0.2
  nh.param("manager/local_segment_length", pp_.local_traj_len_, -1.0);//6.0
  nh.param("manager/control_points_distance", pp_.ctrl_pt_dist, -1.0);//两个控制点之间的距离，0.5
  nh.param("manager/bspline_degree", pp_.bspline_degree_, 3);//B样条的阶数
  nh.param("manager/min_time", pp_.min_time_, false);//true

  bool use_geometric_path, use_kinodynamic_path, use_topo_path, use_optimization, use_active_perception;
  nh.param("manager/use_geometric_path", use_geometric_path, false);//true
  nh.param("manager/use_kinodynamic_path", use_kinodynamic_path, false);//true
  nh.param("manager/use_topo_path", use_topo_path, false);//true
  nh.param("manager/use_optimization", use_optimization, false);//true
  nh.param("manager/use_active_perception", use_active_perception, false);//true

  local_data_.traj_id_ = 0;
  sdf_map_.reset(new SDFMap);
  sdf_map_->initMap(nh);//根据每帧传来的深度图更新地图中点的占据概率
  edt_environment_.reset(new EDTEnvironment);
  edt_environment_->setMap(sdf_map_);//将建立的地图放置在edt_environment_中
  ...
  if (use_active_perception) {
    frontier_finder_.reset(new FrontierFinder(edt_environment_, nh));//前沿检测初始化参数
    heading_planner_.reset(new HeadingPlanner(nh));//好像没用到，后面用到会更新
    heading_planner_->setMap(sdf_map_);//好像没用到，后面用到会更新
    visib_util_.reset(new VisibilityUtil(nh));//好像没用到，后面用到会更新作用
    visib_util_->setEDTEnvironment(edt_environment_);//好像没用到，后面用到会更新
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  ...
  auto& pos = local_data_.position_traj_;//pos是轨迹点
  double duration = pos.getTimeSum();//duration是轨迹花费的总时间

  double dt_yaw = 0.3;
  int seg_num = ceil(duration / dt_yaw);//ceil返回大于x的最小整数，seg_num表示偏航角轨迹的段数
  ...
  for (int i = 0; i < seg_num; ++i) {//遍历每一段轨迹
    double tc = i * dt_yaw;//tc表示每一段轨迹的起始时间
    Eigen::Vector3d pc = pos.evaluateDeBoorT(tc);//pc表示每段轨迹起始的坐标点
    double tf = min(duration, tc + forward_t);//计算超前行走2s后的
    Eigen::Vector3d pf = pos.evaluateDeBoorT(tf);//计算向前行走2s后的点的坐标
    Eigen::Vector3d pd = pf - pc;//两者相减

    Eigen::Vector3d waypt;
    if (pd.norm() > 1e-6) {
      waypt(0) = atan2(pd(1), pd(0));//通过dy/dx求得偏航角
      waypt(1) = waypt(2) = 0.0;
      calcNextYaw(last_yaw, waypt(0));//该语句的作用是将waypt(0)约束在（-π，π）之间
    } else {
      waypt = waypts.back();
    }
    last_yaw = waypt(0);//将计算的
  ...
  Eigen::Vector3d end_yaw(atan2(end_v(1), end_v(0)), 0, 0);//以轨迹最后一点的速度方向计算重点偏航角
  calcNextYaw(last_yaw, end_yaw(0));//将end_yaw(0)约束在（-π，π）之间
