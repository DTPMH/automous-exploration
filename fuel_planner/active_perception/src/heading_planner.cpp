...
HeadingPlanner::HeadingPlanner(ros::NodeHandle& nh) {
  ...
  nh.param("heading_planner/yaw_diff", yaw_diff_, -1.0);//eval 30 * 3.1415926 / 180.0，eval表示将后面表达式的值赋值
  nh.param("heading_planner/lambda1", lambda1_, -1.0);//2.0
  nh.param("heading_planner/lambda2", lambda2_, -1.0);//1.0
  nh.param("heading_planner/half_vert_num", half_vert_num_, -1);//5
  nh.param("heading_planner/max_yaw_rate", max_yaw_rate_, -1.0);//eval 10 * 3.1415926 / 180.0
  nh.param("heading_planner/w", w_, -1.0);//20000
  nh.param("heading_planner/weight_type", weight_type_, -1);//1
