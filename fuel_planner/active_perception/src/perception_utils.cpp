...
PerceptionUtils::PerceptionUtils(ros::NodeHandle& nh) {//都为弧度值
  nh.param("perception_utils/top_angle", top_angle_, -1.0);//0.56125
  nh.param("perception_utils/left_angle", left_angle_, -1.0);//0.69222
  nh.param("perception_utils/right_angle", right_angle_, -1.0);//0.68901
  nh.param("perception_utils/max_dist", max_dist_, -1.0);//4.5
  nh.param("perception_utils/vis_dist", vis_dist_, -1.0);//1.0
  ...
  double hor = vis_dist_ * tan(left_angle_);//水平角度上看见的范围
  double vert = vis_dist_ * tan(top_angle_);//垂直角度上看见的范围
